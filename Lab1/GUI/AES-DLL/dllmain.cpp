...
extern "C" {
    EXPORT void GenerateAndSaveKeys(const int KeySize, const char* OutputFormat, const char* FileName);
    EXPORT void Encryption(const char* Mode, const char* KeyFile, const char* KeyFormat, const char* PlaintextFile, const char* PlaintextFormat, const char* CiphertextFile, const char* CiphertextFormat);
    EXPORT void Decryption(const char* Mode, const char* KeyFile, const char* KeyFormat, const char* RecoveredFile, const char* RecoveredFormat, const char* CiphertextFile, const char* CiphertextFormat);
}
...
void GenerateAndSaveKeys(const int KeySize, const char* OutputFormat, const char* FileName);
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        ...
        cerr << "Usage: " << argv[0] << " <command> [options]\nCommands:\n"
            << "   keygen <KeySize> <OutputFormat> <FileName>" << endl
            ...
            << "Note:\n   KeySize is the size of the key in bits (128, 192, 256)" << endl
            << "   Mode: 'ECB', 'CBC', 'CFB', 'OFB', 'CTR', 'XTS', 'GCM', 'CCM'" << endl
            ...
            << "Example: " << argv[0] << " keygen 256 Base64 key.pem" << endl;
        exit(1);
    }
...
void Encryption(const char* Mode, const char* KeyFile, const char* KeyFormat, const char* PlaintextFile, const char* PlaintextFormat, const char* CiphertextFile, const char* CiphertextFormat) {
    ...
    // Get key size
    size_t KeySize = getKeySize(KeyFile, strKeyFormat);
    ...
    if (strMode == "ECB") {
        ...
        cerr << "Unsupported mode. Please choose:\n\tConfidentiality only: 'ECB', 'CBC', 'CFB', 'OFB', 'CTR' or 'XTS'\n\tConfidentiality and Authentication: 'GCM' or 'CCM'\n";
        exit(1);
    }
